...
#if IS_ENABLED(CONFIG_CHARGER_BQ2518X_MODE_INT_SIG)
#define BQ2518X_CHARGE_CTRL1_CHG_STATUS_INT_MASK 0
#else
...
#if IS_ENABLED(CONFIG_CHARGER_BQ2518X_MODE_INT_SIG)
#define BQ2518X_FLAG0_BUVLO_FAULT BIT(1)
	uint8_t flag0;
	ret = i2c_reg_read_byte_dt(&cfg->i2c, BQ2518X_FLAG0, &flag0);
	if (ret < 0) {
		return ret;
	}
	// LOG_DBG("FLAG0: 0x%02x", flag0);
	if (flag0 & BQ2518X_FLAG0_BUVLO_FAULT) {
		*status = CHARGER_STATUS_NOT_CHARGING;
		LOG_DBG("FLAG0_BUVLO_FAULT -> NOT_CHARGING");
		return 0;
	}
#endif /* IS_ENABLED(CONFIG_CHARGER_BQ2518X_MODE_INT_SIG) */
